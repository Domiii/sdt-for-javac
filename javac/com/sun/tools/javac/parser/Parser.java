...
import com.sun.tools.javac.parser.grammar.GrammarBaseASTNodeDeclTemplate;
import com.sun.tools.javac.parser.grammar.GrammarExpr;
import com.sun.tools.javac.parser.grammar.GrammarNames;
import com.sun.tools.javac.parser.grammar.GrammarToClassConverter;
...
/* ---------- error recovery -------------- */
    ...
    // Domi edit: Make syntax error methods public (not really necessary, but makes things easier)
    public JCErroneous syntaxError(int pos, String key, Object... arg) {
        return syntaxError(pos, null, key, arg);
    }

    public JCErroneous syntaxError(int pos, List<JCTree> errs, String key, Object... arg) {
        setErrorEndPos(pos);
...
//todo: skip to anchor on error(?)
    ...
    // Domi edit: Use variable to write grammar source out to file without having to add a flag to javac
    boolean wroteGrammar;
    ...
    public JCTree.JCCompilationUnit compilationUnit() {
        ...
        // Domi edit: Hacky quick & dirty solution to write grammar source to file
        if (wroteGrammar) {
//            FileWriter w = null;
//            try {
//                w = new FileWriter("../../project/cmmsrc/edu/ntu/compilers/lab4/cmmgrammar/CMMGrammar.java");
//                w.write(toplevel.toString());
//            }
//            catch (Exception e) {
...
//            }
    ...
    /**
     * Domi edit: Type is now Class, Interface, Enum or Grammar
     *
    ...
    JCTree typeDeclaration(JCModifiers mods) {
        ...
        if (mods == null && S.token() == SEMI) {
            ...
            if (S.token() == GRAMMAR) {
                return grammar(mods, dc);
            }
            else {
                return classOrInterfaceOrEnumDeclaration(mods, dc);
            }
        }
    }
    ...
    List<JCTree> classOrInterfaceBody(Name className, boolean isInterface) {
        return classOrInterfaceBody(className, isInterface, new ListBuffer<JCTree>());
    }

    // Domi edit: Overload of classOrInterfaceBody to append to an existing list of defs 
    List<JCTree> classOrInterfaceBody(Name className, boolean isInterface, ListBuffer<JCTree> defs) {
        accept(LBRACE);
        if (S.pos() <= errorEndPos) {
            // error recovery
...
/* ---------- auxiliary methods -------------- */
    ...
    // #############################################################################################################
    // Domi edit: New methods for parsing and converting grammar expressions

    GrammarToClassConverter grammarConverter;
    GrammarToClassConverter getOrCreateGrammarConverter() {
        if (grammarConverter == null) {
            grammarConverter = new GrammarToClassConverter(this, S, F, log, names);
        }
        return grammarConverter;
    }

    /**
     * Grammar = "grammar" Name "{" [ Passes ] "node" Name MembersBody { NonTerminal; } "}"
     *
     * TODO: Improved error detection
     */
    JCStatement grammar(JCModifiers mods, String docComment) {
        int pos = S.pos();
        accept(GRAMMAR);

        Name name = ident();

        if (name.toString().equals("CMMGrammar")) {
            wroteGrammar = true;
        }

        // Token and TokenName types
        accept(LT);
        JCExpression tokenType = type();
        accept(COMMA);
        JCExpression tokenNameType = type();
        accept(GT);

        ListBuffer<GrammarExpr.PassDeclaration> passDecls = new ListBuffer<GrammarExpr.PassDeclaration>();
        ListBuffer<JCExpression> interfaces = new ListBuffer<JCExpression>();
        ListBuffer<JCTree> additionalMembers = new ListBuffer<JCTree>();
        ListBuffer<GrammarExpr.NonTerminal> vars = new ListBuffer<GrammarExpr.NonTerminal>();

        accept(LBRACE);

        // passes
        if (S.token() == GRAMMARPASSES) {
            grammarPassDeclarations(passDecls);
        }

        // members of our Grammar class
        if (S.token() == GRAMMARMEMBERS) {
            S.nextToken();
            grammarImplementingInterfacesAndMemberDeclarationBlock(name, interfaces, additionalMembers);
        }

        // node
        GrammarBaseASTNodeDeclTemplate baseNodeTempl = null;
        if (S.token() == GRAMMARNODE) {
            baseNodeTempl = grammarBaseNode();
        }

        // non-terminals
        do {
            // grammar must have at least one NonTerminal
            vars.append(grammarNonTerminal());
        }
        while (S.token() != RBRACE);

        accept(RBRACE);

        // add Start NonTerminal
        Name startNodeName = names.fromString(GrammarNames.StartNodeClass);
        GrammarExpr.Symbol firstNonTerminalSymbol = new GrammarExpr.Symbol(pos, 0, vars.first().Name);
        vars.prepend(new GrammarExpr.NonTerminal(pos, 0,
                startNodeName,
                List.<JCExpression>nil(),
                List.<JCTree>nil(),
                List.from(new GrammarExpr.Rule[]{           // single Rule of the Start symbol
                        new GrammarExpr.Rule(pos,
                                0,
                                null,
                                List.<JCExpression>nil(),
                                List.<JCTree>nil(),
                                List.from(new GrammarExpr.Symbol[] {firstNonTerminalSymbol}),
                                List.<GrammarExpr.Pass>nil()) })));

        // create GrammarExpr object which is somewhat of a pseudo AST node
        // "pseudo", because I did not actually add nodes for all the new expressions
        // to the original AST hierarchy
        GrammarExpr grammar = new GrammarExpr(pos, name, tokenType, tokenNameType, passDecls.toList(),
                baseNodeTempl, interfaces.toList(), additionalMembers.toList(), vars.toList());

        if (grammar.init(this)) {
            // do not convert if initialization failed
            JCTree.JCClassDecl classDecl = getOrCreateGrammarConverter().convert(grammar, mods);
            attach(classDecl, docComment);
            return classDecl;
        }
        return F.Exec(F.Erroneous());
    }

    void grammarPassDeclarations(ListBuffer<GrammarExpr.PassDeclaration> passDecls) {
        accept(GRAMMARPASSES);
        accept(LBRACE);
        int i = 0;
        while (S.token() != RBRACE) {
            passDecls.append(new GrammarExpr.PassDeclaration(typeOrVoid(), ident(), i++));
            accept(SEMI);
        }
        accept(RBRACE);
    }
    ...
    /**
     * Additional class members and implementing interfaces of any structure inside a grammar declaration.
     * Name [ "implements" TypeList ] "{" ClassBody "}"
     * TODO: Merge code of extra ctors into generated ctors
     */
    void grammarImplementingInterfacesAndMemberDeclarationBlock(Name name, ListBuffer<JCExpression> implementing,
                                                                ListBuffer<JCTree> defs) {
        if (S.token() == IMPLEMENTS) {
            S.nextToken();
            implementing.append(type());
            while (S.token() == COMMA) {
                S.nextToken();
                implementing.append(type());
            }
        }
        classOrInterfaceBody(name, false, defs);
    }

    /**
     * The first (and currently, only) node declaration inside a grammar is the type declaration of the main node.
     * "node" Name ImplementingInterfacesAndMemberDeclarationBlock
     */
    private GrammarBaseASTNodeDeclTemplate grammarBaseNode() {
        accept(GRAMMARNODE);
        ListBuffer<JCExpression> interfaces = new ListBuffer<JCExpression>();
        ListBuffer<JCTree> additionalMembers = new ListBuffer<JCTree>();

        grammarImplementingInterfacesAndMemberDeclarationBlock(null, interfaces, additionalMembers);
        return new GrammarBaseASTNodeDeclTemplate(S.pos(), interfaces.toList(), additionalMembers.toList());
    }

    /**
     * GrammarNonTerminal = Name {ImplementingInterfacesAndMemberDeclarationBlock}: { Rule [ Rule "|" ] } ;
     */
    private GrammarExpr.NonTerminal grammarNonTerminal() {
        int pos = S.pos();
        ...
        // rules
        ListBuffer<GrammarExpr.Rule> rules = new ListBuffer<GrammarExpr.Rule>();

        if (S.token() != SEMI) {
            rules.add(grammarRule(0));

            int i = 1;
            i = afterRule(rules, i);

            while (S.token() != SEMI) {
                rules.add(grammarRule(i++));
                
                i = afterRule(rules, i);
            }
        }
        accept(SEMI);
        return new GrammarExpr.NonTerminal(pos, name, interfaces.toList(), additionalMembers.toList(), rules.toList());
    }

    private int afterRule(ListBuffer<GrammarExpr.Rule> rules, int i) {
        if (S.token() == BAR) {
            S.nextToken();
            if (S.token() == SEMI) {
                // add lambda rule
    ...
    /**
     * GrammarRule = { "[" Name "]" } [ GrammarSymbol ] { GrammarMembers } { GrammarPasses }
     */
    private GrammarExpr.Rule grammarRule(int i) {
        int pos = S.pos();
        Name name;
        ListBuffer<GrammarExpr.Symbol> symbols = new ListBuffer<GrammarExpr.Symbol>();
        ListBuffer<JCExpression> interfaces = new ListBuffer<JCExpression>();
        ListBuffer<JCTree> additionalMembers = new ListBuffer<JCTree>();
        ListBuffer<GrammarExpr.Pass> passes = new ListBuffer<GrammarExpr.Pass>();
        ...
        // optional symbol list & passes
        if (S.token() != BAR && S.token() != SEMI) {
            if (S.token() == IDENTIFIER) {
                int symIndex = 0;
                symbols.add(grammarSymbol(symIndex++));
        ...
        switch (S.token()) {
            ...
            case SEMI:
                break;
            default:
                // Unexpected token: Rules must either be followed by "|" or ";"
                setErrorEndPos(S.pos());
                reportSyntaxError(S.prevEndPos(), "expected2", keywords.token2string(SEMI), keywords.token2string(BAR));
                break;
        }

        return new GrammarExpr.Rule(pos, i, name, interfaces.toList(), additionalMembers.toList(),
                symbols.toList(), passes.toList());
    }

    private void grammarPasses(ListBuffer<GrammarExpr.Pass> passes) {
        while (S.token() == LPAREN) {
            ...
            if (S.token() != RPAREN) {
                // based on Parser.formalParameters()
...
//            // thrown (makes pass propagation more complicated)
//            List<JCExpression> thrown = List.nil();
//            if (S.token() == THROWS) {
//                S.nextToken();
//                thrown = qualidentList();
//            }
            ...
            // code
            JCBlock code = block();
            passes.append(new GrammarExpr.Pass(pos, name, params.toList(), code));
        }
    }
     ...
     * @return
     */
    private GrammarExpr.Symbol grammarSymbol(int index) {
        int pos = S.pos();
        Name name = ident();

        return new GrammarExpr.Symbol(pos, index, name);
    }

    // Domi edit: Utility methods

    JCExpression typeOrVoid() {
        boolean isVoid = S.token() == VOID;
        JCExpression type;
        if (isVoid) {
            type = to(F.at(S.pos()).TypeIdent(TypeTags.VOID));
            S.nextToken();
        } else {
            type = type();
        }
